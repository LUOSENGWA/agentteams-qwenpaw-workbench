...
Worker-dimension tree (design): Leader and Workers are all Workers;
each Worker owns its spawn tree. Main data source = Controller spawn endpoint
(`GET /api/v1/projects/{id}/spawns`, ) once merged; until then the
...
# 「团队桥接」（"看 spawn 树"跳转定位）待做时消费；v0.2.0 骨架期即为此预埋。
...
def infer_worker_role(display_name: str, mxid: str) -> str:
    ...
    if re.search(r"manager", name):
        return "leader"  # Manager handles team orchestration + project group creation
    return "worker"
