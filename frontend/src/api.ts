 ...
 * 实锤：生产 homeserver（Tuwunel/Conduit 系，部分实现）不识别
...
/** Controller workflowInterrupt（上游已合并）：paused 项目带
...
export async function fetchWorkflowProjects(): Promise<{
  ...
  try {
    ...
    // 按真实状态码分类（此前字符串匹配 msg.includes("401")——
...
/** 项目写操作（上游已合并：pause/resume/replan/complete + 任务级 cancel，
...
/** 更新现有 Worker 的模型（PUT /workers/{name} 合并语义——只应用非空字段，
...
/** 房间重命名（m.room.name state 事件；需房间权限表授权
...
/** 房间权限诊断（改名 403 根因展示）。
 * 实测：Controller/Manager 创建的房间 m.room.power_levels 状态
...
// ── 宿主通知收件箱（通知中心）：/console/inbox 事件流 ─────
