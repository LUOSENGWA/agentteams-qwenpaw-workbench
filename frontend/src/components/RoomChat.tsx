...
function MessageBody({
  ...
  // workflow 卡片（dashboard normalize 规则 1 同款优先级=最高）：
...
export default function RoomChat(props: RoomChatProps) {
  ...
  // 搜索跳转定位区（修复）：目标事件不在已加载窗口（搜索结果多为旧消息）
  ...
  // 跨房间搜索跳转：jumpToEventId 变化后轮询定位——切房时消息列表
  ...
  return (
    ...
    >
      ...
      {/* 房间内消息搜索：Drawer 面板，房间过滤 + 上下文预览 + 定位 */}
      {room ? (
        <MessageSearch
          open={searchOpen}
