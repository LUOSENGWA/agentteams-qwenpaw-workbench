...
| Prerequisite | No API can issue it (upstream security design) — provided offline by the deployment admin; **the token lives on tmpfs — every controller container restart mints a new one** (app.go: "free token rotation on every container start"), a pasted token is a snapshot — after a controller restart/rotation, re-fetch via the "Token fetch command" section and paste again | Higress URL = **required** (v0.5.0-beta.12 decision: the Console shares the Controller's container on a different port, but the host port is operator-chosen at install time — `AGENTTEAMS_PORT_CONSOLE`, default 18001, differs per deployment → the 8001/6868 blind probe was removed; an empty value returns an actionable error) |
