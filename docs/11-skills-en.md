...
**Depends on the upstream skill endpoint merging**: until then a 404 placeholder card shows (expected, not a fault).
...
"Host skills" = **this machine's own QwenPaw instance SkillPool** (the host's skills), not the Workers' team skills — renamed from "Skills" in v0.5.0-beta.12 to disambiguate, with a quick entry on the home page.
...
- The skill catalog shows a placeholder card: it lights up automatically once the upstream endpoint merges — no plugin upgrade needed
