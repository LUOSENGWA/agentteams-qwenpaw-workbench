...
- **Team list + Worker tree**: each team shows its real Worker structure (spawn tree filled from the source of truth); Worker cards show runtime status/phase; **spawn-tree nodes show "Allowed tools / Allowed skills" tags** (v0.5.0-beta.13: the per-dispatch tool narrowing is persisted by the Worker runtime and surfaced through the Controller spawn endpoint; **rendered only when the data is available — zero noise otherwise**; visibility conditions in [Controller version compatibility](./15-controller-compatibility-en.md))
...
- **Model dropdown = union** (since v0.5.0-beta.12, extended to the Manager table in v0.5.0-beta.12): all three entry points (create-worker dialog / team-create row / team-config inline) + the Manager table model column (v0.5.0-beta.12) share one candidate union (`../modelUnion`, same pre-write validation) = ① gateway aliases (route-resolvable, fetched live from Higress AI routes + providers via the L1 password-mode Console session) ② builtin aliases (the 16 official AgentTeams aliases) ③ serving SGLang ∪ in-use models; without a Console session the alias layer hides, **with an explicit hint in token mode** (v0.5.0-beta.12: amber notice at the CRD panel top + above the Manager table — configure admin account+password to make it visible, or wait for P1-3), free input unchanged
...
- **Detail panel (row expand)** (since v0.5.0-beta.12, ): expanding a row shows image / version / MXID / personal room + **DM (jumps to the personal room)** + **Logs** (L1, last 300 lines, via the existing docker-logs proxy, zero new endpoints)

## Channels

**Worker channel configuration** (lights up automatically once the upstream AgentTeams channel endpoint merges; until then a 404 placeholder card — expected):
